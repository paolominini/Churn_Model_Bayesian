...
df['is_return']= df['InvoiceNo'].astype(str).str.upper().str.startswith('C')
...
# --- Frequency ---
frequency = (purchases.groupby('CustomerID')['InvoiceNo']
             .nunique()
             .rename('Frequency'))
...
# --- Cancellation Rate ---
total_inv= df.groupby('CustomerID')['InvoiceNo'].nunique().rename('Total_Inv')
cancel_inv = returns.groupby('CustomerID').nunique().rename("Cancel_Inv")
...
avg_time = (purchases.groupby("CustomerID")['InvoiceData']
            .apply(std_gap)
